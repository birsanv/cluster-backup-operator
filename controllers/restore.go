...
func getLatestVeleroRestores(
	...
	for i := range veleroRestoreList.Items {
		restoreName := veleroRestoreList.Items[i].Name
		// Include if exact match
		if currentRestoreNames[restoreName] {
			latestRestores = append(latestRestores, veleroRestoreList.Items[i])
			continue
		}
		// Include if base name matches (handles -active variants)
		baseNameWithoutActive := strings.TrimSuffix(restoreName, "-active")
		if baseRestoreNames[baseNameWithoutActive] {
			latestRestores = append(latestRestores, veleroRestoreList.Items[i])
		}
	}
